...
AD.read["Path"] = function(obj, str, key) {
  ...
  if (txtu != 'txtu') {
    Error.runtimeError(ActionStream.ERROR_CODE, "Bad path error");
  }
...
ActionFile.prototype.write = function(fptr) {
  ...
  self.actionSet.writeHeader(str, true);
  file.write(str.toStream());

  var acts = self.actionSet.actions;
...
ActionSet.prototype.read = function(str, readVersion) {
  ...
  ActionStream.log("ActionSet.read(" + self.name + ")");
...
function main() {
  ...
  //Stdlib.log("Start");
  //Stdlib.log("Revision: $Revision$");
  //Stdlib.log("App: " + app.name);
  //Stdlib.log("App Version: " + app.version);
  //Stdlib.log("OS: " + $.os);

  var ui = new SLCToATNUI();
  //var process = new TextProcessor();
  //process.exec('~/desktop/ScriptingListenerJS.log', '~/desktop/temp.atn');
  ui.process({source:'~/desktop/ScriptingListenerJS.log', dest:'~/desktop/PhotoshopRecorder/' + file_num + '/tmp/ActionListener.atn'});
};
...
// PSString._add("enumerated", "enumerated"); // DON'T NAME THINGS "enumSomething"; just name them "something");
...
SLCFix.handleLine = function(line, index, outputBuffer) {
  ...
  // sway out the SL var usages with our symbols
  if ((m = line.match(/id\w+/g)) != null) {
    for (var i = 0; i < m.length; i++) {
      line = line.replace(m[i], fixer.idMap[m[i]]);
    }
  }
